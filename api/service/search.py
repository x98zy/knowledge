...
class SearchService:
    ...
    async def full_text_search(
        ...
        if not datasets:
            raise CustomException("知识库不存在")
        search_docs = await asyncio.gather(
            *[VectorFactory(dataset).full_text_search(query, top_k, score, query_filter) for dataset in datasets]
        )
        docs: list[SearchDocument] = []
        for doc_list in search_docs:
            docs.extend(doc_list)
    ...
    async def hybrid_search(
        ...
        if not datasets:
            raise CustomException("知识库不存在")
        search_docs = await asyncio.gather(
            *[VectorFactory(dataset).hybrid_search(query, top_k, score, query_filter) for dataset in datasets]
        )
        docs: list[SearchDocument] = []
        for doc_list in search_docs:
            docs.extend(doc_list)
