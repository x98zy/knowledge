...
from core.llm_generator.prompts import GENERATOR_QA_PROMPT, SPLIT_FILE_CONTENT_PROMPT
...
class LLMGenerator:
    ...
    async def split_file_content(self, content: str) -> str:
        prompt = SPLIT_FILE_CONTENT_PROMPT
        prompt_messages: list[PromptMessage] = [SystemPromptMessage(content=prompt), UserPromptMessage(content=content)]
        model_manager = ModelManager(
            settings.QA_MODEL_NAME,
            ModelProvider.OPENAI,
